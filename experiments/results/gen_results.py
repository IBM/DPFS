...
def advanced_latency_extra():
    ...
    for i in range(1, 4):
        ...
        for (rw, val) in null_avg.items():
            ...
            if i == 3:
                err=None
            rects = ax.bar(i + offset, val, yerr=err, width=width, bottom=0, capsize=5, color=colors[rw], edgecolor='black', hatch='/')
            multiplier += 1

    fig.tight_layout()
    ax.grid(which='both', linestyle='dashed', linewidth='1')
    ax.set_axisbelow(True)
    ax.set_ylim(top=150)
